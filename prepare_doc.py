...
# Fetch embedded_movies
df_embedded = run_query("""
SELECT _id, plot_embedding
FROM embedded_movies;
""")

# Fetch theaters info
df_theaters = run_query("""
...
comments_grouped = df_comments_users.groupby('movie_id')['review_full'].apply(lambda x: '\n\n'.join(x)).reset_index()
...
theaters_grouped = df_theaters.groupby('_id').apply(lambda group: '\n'.join(group.apply(format_theater, axis=1))).reset_index()
theaters_grouped.rename(columns={0: 'theater_info'}, inplace=True)

# Merge movies and embedded_movies
...
# Merge with theaters
df_final = pd.merge(df_temp, theaters_grouped, left_on='_id', right_on='_id', how='left')
...
print(f"Created {len(df_final_docs)} full movie documents (with embedded flag, ratings, awards, theaters & reviews).\n")
